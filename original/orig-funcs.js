...
const broadcastIntervalMessages = (channel) => {
  ...
  channel.assertExchange(
    ORIG_CONSTANTS.BROADCAST_MESSAGE_CHANNEL,
    ORIG_CONSTANTS.BROADCAST_MESSAGE_TYPE,
    {
      durable: false,
    }
  );
  ...
  const timer = setInterval(() => {
    ...
    if (State.getState() === State.RUNNING_STATE) {
      ...
      channel.publish(
        ORIG_CONSTANTS.BROADCAST_MESSAGE_CHANNEL,
        ORIG_CONSTANTS.BROADCAST_MESSAGE_TOPIC,
        Buffer.from(sentMsg)
      );
...
const subscribeToStateUpdateChannel = (
  channel,
  connection,
  intervalMessages
) => {
  channel.assertExchange(
    ORIG_CONSTANTS.STATE_UPDATE_CHANNEL,
    ORIG_CONSTANTS.STATE_UPDATE_TYPE,
    {
      durable: false,
    }
  );
